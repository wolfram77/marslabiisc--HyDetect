...
for(int i=(*dev1_community).g.nb_nodes;i<(*dev1_community).g.nb_nodes+newV1+1;i++)	
		stat1[i]=0;
		...
		for(unsigned int j=adj3;j<adj4;j++)
			{
				//if(stat1[c[i]]>0)
				stat1[c[i]]-=1;
			//	if(stat1[c[edges[j]]]>0)
				stat1[c[edges[j]]]-=1;
				//(weight2[c[i]]>0)
			//	weight2[c[i]]-=1;
			//	if(weight2[c[edges[i]]]>0)
                          //      weight2[c[edges[j]]]-=1;
...
for(unsigned int i=0;i<gpunodes-1;i++)
{      // stat1[i+1]+=stat1[i];
	//stat2[i]=stat1[i];
     cout<<stat1[i]<<" ";
}
for(unsigned int i=0;i<gpunodes;i++)
{  
	stat2[i]+=stat1[i];
	cout<<stat2[i]<<" ";
}
