...
int* Community::compute_next_graph(cudaStream_t *streams, int nrStreams,
...
int *nc=(int *)malloc(sizeof(int)*g.nb_nodes);
for(int i=0;i<g.nb_nodes;i++)
        nc[i]=0;
/*thrust::device_vector<int> ncg(g.nb_nodes);
update_nodes_based_on_new_CID << < nr_of_block, NR_THREAD_PER_BLOCK>>>
        ...
        thrust::raw_pointer_cast(n2c_new.data()),thrust::raw_pointer_cast(ncg.data())   );
    thrust::copy(ncg.begin(), ncg.end(), nc);
*/
