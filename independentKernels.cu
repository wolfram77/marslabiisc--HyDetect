...
__global__
void update_nodes_based_on_new_CID(int* comm_nodes, int* pos_ptr_of_new_comm,
        int* oldToNewCidMapping, int* n2c, int nb_nodes,int *n2c_new,int *ncg) {

    int vid = threadIdx.x + blockIdx.x * blockDim.x;
    while (vid < nb_nodes)
        {
                        int old_cid = n2c[vid];
                        ncg[vid]=n2c_new[old_cid];
		//	printf("value of ncg=%d",ncg[vid]);
