	...
	/*	if(dirty2[i]==true)
			{
				...
				for(unsigned int j=adj1; j<adj2; j++){
					G->bord[(vtxInd[j].tail+1)]=true;
					if((vtxInd[j].tail+1)!=G->numVertices){
					unsigned int adj3=G->edgeListPtrs[(vtxInd[j].tail+1)];
					unsigned int adj4=G->edgeListPtrs[(vtxInd[j].tail+1)+1];
