...
for(long i=0;i<newV1;i++){

mo1[i].vertex=0;
mo1[i].edgeno=0;
mo1[i].edgeListPtrsM=0;
mo1[i].edgesM=NULL;
}
...
{
        mo1[i].edgesM=(edge*)malloc(sizeof(edge)*noedge[i]);
for(int i=0;i<noedge[i];i++)
{

                mo1[i].edgesM[ij].head=*it;
                mo1[i].edgesM[ij].tail=*(it+1);
                mo1[i].edgesM[ij].weight=*(it+2);
        ij++;
}
}
}
